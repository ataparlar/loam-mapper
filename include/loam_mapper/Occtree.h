...
public:
    ...
    void addPoint(const PointType &point) {
        octree->addPointToCloud(point, cloud);
    }
    ...
    CloudPtrType box(PointType pt_search, float width) {
        ...
        Eigen::Vector3f pt_min(pt_search.x - box_radius,
                               pt_search.y - box_radius,
                               pt_search.z - box_radius);
        Eigen::Vector3f pt_max(pt_search.x + box_radius,
                               pt_search.y + box_radius,
                               pt_search.z + box_radius);
